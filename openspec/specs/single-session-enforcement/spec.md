# Single-Session Enforcement Capability

## Purpose

This capability defines the rules for ensuring only one active session exists per user at any given time. It covers the data model (`activeSessionToken` on the user record), login rejection when a session is already active, session release on logout, automatic session release on Socket.IO disconnect (with a grace period for page refreshes), and testability requirements.
