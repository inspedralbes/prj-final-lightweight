            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-white">
                  {t("virtualGym.roomTitle")}
                </h1>
                <p className="text-gray-400 mt-1">
                  {t("virtualGym.roomSubtitle")}
                </p>
              </div>
            </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 md:p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <p className="text-xs text-gray-500 uppercase font-black tracking-widest mb-1">
                {t("virtualGym.roomCode")}
              </p>
              <span className="text-2xl md:text-3xl text-white font-bold font-mono">
                {roomId}
              </span>
            </div>
            <div className="flex gap-2 w-full md:w-auto">
              <div className="px-3 py-2 bg-zinc-800 rounded-lg text-white text-xs font-bold">
                {t("virtualGym.usersOnlineCount", {
              ...
              <div className="px-3 py-2 bg-orange-500/10 border border-orange-500/20 rounded-lg text-orange-500 text-xs font-black uppercase flex items-center">
                {isHost ? t("virtualGym.host") : t("virtualGym.guest")}
              </div>
              <button
                onClick={onLeave}
        ...
        <div className="max-w-4xl mx-auto py-6 text-center space-y-6">
          {!isHost && (
            <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-2xl animate-in slide-in-from-bottom duration-700">
              {usersInRoom.length < 2 ? (
                <>
                  <div className="w-10 h-10 rounded-full border-4 border-zinc-700 border-t-orange-500 animate-spin mx-auto mb-3" />
                  <p className="text-zinc-300 font-bold text-base md:text-lg uppercase tracking-tighter italic">
                    {t("virtualGym.guestWaitingForHost")}
                  </p>
                  <p className="text-zinc-500 mt-2 text-sm">
                    {t("virtualGym.guestWaitingForHostSubtitle")}
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-zinc-300 font-bold text-base md:text-lg uppercase tracking-tighter italic">
                    {t("virtualGym.hostConfiguring")}
                  </p>
                  <p className="text-zinc-500 mt-2 text-sm">
                    {t("virtualGym.hostConfiguringSubtitle")}
                  </p>
                </>
              )}
            </div>
          ...
          {isHost && (
            <div className="space-y-3">
              <h2 className="text-2xl md:text-3xl font-black text-white uppercase italic tracking-tighter">
                {t("virtualGym.cooperativeTraining")}
              </h2>
              <p className="text-zinc-500 max-w-lg mx-auto text-xs md:text-sm">
                {t("virtualGym.cooperativeSubtitle")}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="bg-zinc-900 border border-zinc-800 w-full max-w-2xl rounded-3xl shadow-2xl relative overflow-hidden flex flex-col max-h-[85vh] animate-in fade-in zoom-in duration-300">
              <div className="p-8 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50">
                <div>
                  <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter">
                    {t("virtualGym.selectRoutine")}
                  </h3>
                  <p className="text-zinc-500 text-sm mt-1">
                    {t("virtualGym.selectRoutineSubtitle")}
                  </p>
                </div>
                <button
                  onClick={() => setIsRoutineModalOpen(false)}
              ...
              <div className="flex border-b border-zinc-800 p-2 bg-black/20">
                <div className="flex-1 py-3 text-xs font-black uppercase tracking-widest transition-all rounded-xl flex items-center justify-center gap-2 bg-orange-500 text-white shadow-lg shadow-orange-500/10">
                  <UserIcon className="w-4 h-4" />
                  {t("virtualGym.myRoutinesTab")}
                </div>
              </div>
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="text-left">
                            ...
                            <p className="text-sm text-zinc-500 font-medium">
                              {t("virtualGym.exercisesScheduled", {
              ...
              <div className="p-8 border-t border-zinc-800 bg-zinc-900/50 flex justify-end gap-6">
                ...
                >
                  {t("virtualGym.startSessionButton")}
                </button>
              </div>
            </div>
          </div>
