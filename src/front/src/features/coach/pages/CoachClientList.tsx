...
import {
  coachClientService,
  type Client,
} from "@/features/coach/services/coachClientService";
import { useTranslation } from "react-i18next";
import { useToast } from "@/shared/hooks/useToast";
import { Mail, Edit, X, MessageCircle } from "@/shared/components/Icons";
...
const Clients = () => {
  ...
  const fetchClients = async () => {
    try {
      setLoading(true);
      const data = await coachClientService.getClients();
      setClients(data);
    } catch (error) {
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      setSavingNotes(true);
      await coachClientService.updateClient(selectedClient.id, {
  ...
  const handleUnlinkClient = async () => {
    ...
    setUnlinking(true);
    try {
      await coachClientService.unlinkClient(confirmUnlinkClient.id);
      toast.success(
        ...
        {clients.length === 0 && !loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {clients.map((client) => (
              <div
                key={client.id}
                className="bg-[#1a1a1a] rounded-lg border border-[#2a2a2a] p-6 hover:border-orange-500/50 transition-colors flex flex-col"
              >
                {/* Client Header */}
                <div className="mb-4">
                ...
                <div className="mb-4 p-3 bg-[#0a0a0a] rounded border border-[#2a2a2a] flex-1">
                  <p className="text-xs text-gray-500 mb-1">
                    {t("clients.privateNotes")}
                  </p>
                  <p className="text-sm text-gray-300 line-clamp-2 min-h-[1.25rem]">
