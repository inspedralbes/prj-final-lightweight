...
import type { ChatNotification } from "../context/NotificationContext";
...
const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  const { notifications, markAsRead, removeNotification, unreadCount } =
    useNotification();
  ...
  const handleChatClick = (roomId: string, id: string) => {
    markAsRead(id);
    onChatClick?.(roomId);
    setExpanded(false);
  };
  ...
  const renderNotificationCard = (
    ...
    return (
      <div
        ...
        onClick={() => handleChatClick(notification.roomId, notification.id)}
      >
  ...
  return (
    <div className="fixed top-20 right-4 space-y-2 z-50 max-w-sm">
      ...
      {/* Lista flotante de notificaciones sin expandir */}
      {!expanded &&
        chatNotifications
          .slice(0, 2)
          .map((notification) => renderNotificationCard(notification, true))}
    </div>
  );
};
